...
import {patientsArr} from './patient.js'
...
//==========================
    ...
    const emailInputLoc = document.querySelector("input")
    const btnLoc = document.querySelector(".btn")
    const questionsContLoc = document.querySelector(".questions-container")
    const entryWrapperLoc = document.querySelector(".entry-wrapper")

    const showQuestionnaire = ()=> {
        document.querySelector(".questionnaire-page").style.display = "block"
        console.log("OK")
    }

    btnLoc.addEventListener("click", ()=> {
        if (emailInputLoc.value) {
            patientsArr.forEach((elem)=> {
                if (elem === emailInputLoc.value) {
                    questionsContLoc.style.opacity = "1"
                    entryWrapperLoc.style.opacity = "0"
                    entryWrapperLoc.style.zIndex = "-99"
